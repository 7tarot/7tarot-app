...
// 7TAROT — One Drag & Drop (Shop + Raffle, image optional)
const RAFFLE = {
  ...
  endsAt: '2025-09-10T11:28:00+01:00', // Wed, 10 Sept 2025, 11:28 BST
  image: '' // leave blank to HIDE the image box (no assets needed)
};
...
function renderProducts(items, currency='GBP', locale='en-GB') {
  ...
  for (const p of items) {
    ...
    card.innerHTML = `
      ...
      <div class="cta-row">
        ${soldOut ? `<a class="btn" href="${url}" target="_blank" rel="noopener">View (Sold Out)</a>` :
          `<a class="btn" href="${url}" target="_blank" rel="noopener">Buy</a>`}
      </div>
    `;
...
function renderRaffle() {
  ...
  const imgWrap = document.querySelector('#raffle .img');
  const top = document.querySelector('#raffle .top');
  const title = $('#raffle-title-text');
  const ends = $('#raffle-ends');
  const enter = $('#raffle-enter');
  const copy = $('#raffle-copy');

  if (!title || !ends || !enter || !copy) return;
  ...
  // Hide image block entirely if no image set
  if (!RAFFLE.image) {
  ...
  if (end && !isNaN(end.valueOf())) {
    const tick = () => {
      ...
      ends.textContent = `Ends: ${humanDate(RAFFLE.endsAt)} • ${d}d ${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;
